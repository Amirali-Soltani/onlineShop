...
import FirstBox from "./../last_category/top__first__box";
import SecondBox from "./../last_category/top__second__box";
import WholeProduct from "./../last_category/products";
import Pagination from "./../last_category/pagination__box";
...
class Last_category extends Component {
  ...
  componentDidUpdate(prevProps, prevState) {
    if(prevProps.match.params.word!==this.props.match.params.word) {
      let originalSearch=this.props.match.params.word
      if(originalSearch!==undefined){
        this.handleExternalSearch(originalSearch)
      }
...
//       this.handleExternalSearch(originalSearch)
//     }
//     if(originalSearch==undefined)this.setState({originalSearch:""});
...
//     this.setState({ allCategories });
    ...
    let categoryId = this.props.match.params.id;
    if(this.props.match.params.filters!==undefined) this.importInitialURL();
    categoryId = parseInt(categoryId);
    let products = this.props.productsOfSpecificCategory(categoryId);
    ...
    let PresentURL={...this.state.PresentURL}
      PresentURL.id=categoryId
      PresentURL.stringFilter=this.props.match.params.filters
    this.setState({ categoryId, products, allAttributeItemS, allCategories,PresentURL });
  // }
  }
  ...
  handelClickSearch=(searchWord)=>{
    ...
    let newStringFilter=""
      ...
      if(index!==-1){
        ...
        newStringFilter=stringFilter.slice(0,index+length)+`${searchWord}`+stringFilter.slice(endOfIndexOldSearchWord+index)
        }
        else{
          newStringFilter=stringFilter.slice(0,index+length)+`${searchWord}`
  ...
  handelClickOrder=(order)=>{
    ...
    let newStringFilter=""
      ...
      if(index!==-1){
        ...
        let endOfIndexOldSearchWord=sliceStringFilter.indexOf("&")
        if(endOfIndexOldSearchWord!==-1){
        newStringFilter=stringFilter.slice(0,index+length)+`${order.path}_${order.sort}`+stringFilter.slice(endOfIndexOldSearchWord+index)
        }
        else{
          newStringFilter=stringFilter.slice(0,index+length)+`${order.path}_${order.sort}`
  ...
  handelAllAttributesClick=( nameOfAttribute,indexOfAttribut,id,titleItem,item)=>{
    let {PresentURL}=this.state
    this.handleAllAttributes(nameOfAttribute,indexOfAttribut,id,titleItem,item)
...
let allAttributesSelected = { ...this.state.allAttributesSelected };
    ...
    let filteredByDiffferentsAttributes=[...this.state.filteredByDiffferentsAttributes]
    let indexHolder = {...this.state.indexHolder};
    allAttributesSelected={}
    specificSelectedItems=[]
    filteredByDiffferentsAttributes=[]
    indexHolder=[]
    this.setState({ data,RangePriceClassName,filterdByPrice,stock,filteredByStock,discount,filteredByDiscount,colors,listOfColors,filteredByColor,allAttributesSelected,specificSelectedItems,filteredByDiffferentsAttributes,indexHolder });
...
console.log(PresentURL);
    filters.map((filter) => {
      ...
      if (filter.slice(0, 5) === "range") {
        ...
        price = price.split("_");
        console.log(price,"ssssssssssssssssssssssss",price.length);
        price.map((p) => {
          if( p !== ""){
          setTimeout(() => {
...
if(boolean==="true"){
  ...
  importURlColors=(initialColors)=>{
      ...
      for(let key in initialColors ){
        allColors.map(color=>{
        if(color.class===initialColors[key]) this.handleColor(color.class,color)
      })
  ...
  handleSearch = (searchWord) => {
    let { products } = this.state;
      let filtered=products.filter(m =>(m.title.toLowerCase().includes(searchWord.toLowerCase()) || m.title_En.toLowerCase().includes(searchWord.toLowerCase())));
  ...
  handleFilterOfPrice = (number) => {
    ...
    let { products, filterdByPrice } = this.state;
    let prices = [];
  ...
  handleColor = (colorName, c) => {
    ...
    this.resetPaginate()
    this.setState({ filteredByColor: newProducts });
  };
...
//   };
  ...
  getProducts = () => {
    ...
    if (filteredByColor.length === 0) filteredByColor = products;
    if (
...
sortproducts=()=>{
 ...
 if(Sort.order==='asc')deletedZeroes= deletedZeroes.sort((a,b)=>a-b)
...
let primitiveSort=[]
 deletedZeroes.map(number=>{
   for (let index = 0; index < products.length; index++) {
     if(number===products[index].price || number===products[index].price * (100 - parseInt(products[index].off)) / 100) 
...
let paginated = products.slice((currentPage-1)*pageSize,currentPage*pageSize)
return paginated
}
resetPaginate=()=>{
  let pagination ={...this.state.pagination}
...
this.setState({pagination});
  ...
  render() {
    return (
      <Header__Footer history={this.props.history}>
        <div className="main">
          ...
          <div className="whole__category">
            <div className="container">
              <div className="main__search">
                ...
                originalSearch={this.state.data.internalSearchBox}
                ...
                <div className="left__search__products">
                  <div className="whole__left__search">
                    ...
                    deletFilterOfDiscountStock={this.deletFilterOfDiscountStock}
                    RangePriceClassName={this.state.RangePriceClassName}
                    handleFilterOfPrice={this.handleFilterOfPrice}
